...
import {
  ...
  FaLayerGroup, FaGlobeAsia, FaChartBar, FaCog, FaSearch,
  FaBell, FaPlus, FaArrowUp, FaShoppingCart, FaPalette,
  FaSignOutAlt, FaChevronRight, FaChevronDown, FaTimes,
...
const NAV_ITEMS = [
  ...
  { key: "orders", label: "Orders", icon: FaShoppingCart, children: [{ key: "drafts", label: "Drafts" }] },
  { key: "products", label: "Products", icon: FaBoxOpen, children: [
...
const SETTINGS_ITEMS = [
  { key: "settings-domains", label: "Domains", icon: FaGlobe },
];
...
export default function Dashboard() {
  ...
  const renderNavGroup = (items) =>
    items.map(({ key, label, icon: Icon, children }) => {
      ...
      return (
        <div key={key}>
          <button
            onClick={() => { selectItem(key); if (hasChildren) toggleMenu(key); }}
          ...
          >
            ...
            {hasChildren && (
              <FaChevronDown size={10} className={`transition-transform shrink-0 ${isOpen ? "rotate-180" : ""}`} />
            )}
          </button>
          {hasChildren && isOpen && (
            <div className="ml-[26px] border-l border-white/10 pl-3 mt-1 mb-1 space-y-0.5">
              {children.map((child) => (
                <button key={child.key} onClick={() => selectItem(child.key)}
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition ${
                    active === child.key ? "bg-white/10 text-white" : "text-white/50 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  {child.label}
                </button>
  ...
  const renderContent = () => {
    if (active === "settings-domains") return <DomainSettings />;
    // Default home
    return (
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col">
            <h3 className="font-semibold text-gray-800 text-lg">Add your first product</h3>
            <p className="text-gray-500 text-sm mt-2 flex-1">Start with a title, price, and a photo. You can always add more detail later.</p>
            <button onClick={() => selectItem("products")}
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col">
            <h3 className="font-semibold text-gray-800 text-lg">Choose your store design</h3>
            <p className="text-gray-500 text-sm mt-2 flex-1">Pick a theme that fits your brand, then customize from there.</p>
            <button onClick={() => selectItem("online-store")}
              className="mt-5 inline-flex items-center gap-2 bg-black hover:bg-gray-900 text-white text-sm font-semibold px-4 py-2.5 rounded-xl w-fit transition">
          ...
          {showTrial && isTrial && (
            <div className="bg-white rounded-2xl shadow-sm p-6 sm:col-span-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">Select a plan to start accepting orders and payments.</p>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <button onClick={() => setShowTrial(false)} className="text-gray-500 hover:text-gray-700 text-sm font-medium px-3 py-2.5">Skip for now</button>
    ...
    <>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        <div className="pt-4 mt-4 border-t border-white/10">
          <p className="px-3 pb-2 text-[11px] uppercase tracking-wide text-white/30 font-semibold">Sales channels</p>
          {renderNavGroup(SALES_CHANNELS)}
        </div>
        <div className="pt-4 mt-4 border-t border-white/10">
          <p className="px-3 pb-2 text-[11px] uppercase tracking-wide text-white/30 font-semibold">Apps</p>
          {renderNavGroup(APPS_SECTION)}
        </div>
        <div className="pt-4 mt-4 border-t border-white/10">
          <button onClick={() => { selectItem("settings"); toggleMenu("settings"); }}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition ${
              active.startsWith("settings") ? "bg-white/10 text-white" : "text-white/60 hover:bg-white/5 hover:text-white"
            }`}
          >
            <FaCog size={15} />
            <span className="flex-1 text-left">Settings</span>
            <FaChevronDown size={10} className={`transition-transform shrink-0 ${openMenus["settings"] ? "rotate-180" : ""}`} />
          </button>
          {openMenus["settings"] && (
            <div className="ml-[26px] border-l border-white/10 pl-3 mt-1 space-y-0.5">
              {SETTINGS_ITEMS.map(({ key, label }) => (
                <button key={key} onClick={() => selectItem(key)}
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition ${
                    active === key ? "bg-white/10 text-white" : "text-white/50 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  {label}
                </button>
  ...
  return (
    <div className="h-screen w-screen flex bg-[#f1f2f2] overflow-hidden">
      <aside className={`hidden md:flex shrink-0 bg-[#071311] text-white/80 flex-col transition-all duration-300 overflow-hidden ${sidebarOpen ? "w-64" : "w-0"}`}>
        <div className="w-64 h-full flex flex-col">{SidebarContent}</div>
      </aside>
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40 flex">
          <div className="w-64 bg-[#071311] text-white/80 flex flex-col h-full">{SidebarContent}</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 shrink-0 bg-[#071311] flex items-center justify-between px-4 sm:px-6 gap-3 sm:gap-4">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <button onClick={() => setMobileOpen(true)} className="md:hidden text-white/70 hover:text-white shrink-0"><FaBars size={18} /></button>
            <button onClick={() => setSidebarOpen((v) => !v)} className="hidden md:inline-flex text-white/70 hover:text-white shrink-0"><FaBars size={16} /></button>
            <div className="flex-1 max-w-xl min-w-0">
              <div className="relative">
                ...
                <span className="hidden sm:inline absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-white/30 border border-white/20 rounded px-1.5 py-0.5">Ctrl K</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3 sm:gap-4 shrink-0">
            <button className="text-white/70 hover:text-white"><FaBell size={16} /></button>
            <div className="flex items-center gap-2 cursor-pointer">
              <div className="w-8 h-8 rounded-full bg-green-600 flex items-center justify-center text-white text-xs font-bold">{initials}</div>
              <span className="text-white text-sm font-medium hidden sm:inline">{storeName}</span>
            </div>
          </div>
        </header>
